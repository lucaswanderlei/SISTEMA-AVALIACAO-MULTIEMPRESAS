...
export function UserAccessManager() {
  ...
  const createUser = async (e: React.FormEvent) => {
    ...
    setBusyId('new');
    const result = await apiCreateUser({ name: name.trim(), login: login.trim().toLowerCase(), email: email.trim(), password, accessLevel });
    setBusyId(null);
  ...
  const saveEdit = async (e: React.FormEvent) => {
    ...
    const result = await apiUpdateUser(editing.id, {
      name: editName.trim(),
      login: editLogin.trim().toLowerCase(),
    ...
    <div className="p-5 sm:p-6 bg-white rounded-3xl border border-stone-200 shadow-sm space-y-4">
      ...
      {showCreate && (
        <form onSubmit={createUser} className="p-4 rounded-2xl bg-sky-50/60 border border-sky-100 grid grid-cols-1 md:grid-cols-2 gap-3">
          ...
          <input required minLength={3} value={login} onChange={e=>setLogin(e.target.value.toLowerCase())} placeholder="Login" className="p-3 rounded-xl border border-stone-300 bg-white text-sm" />
          ...
          <div key={user.id} className="p-4 rounded-2xl border border-stone-200 bg-stone-50/60">
            {editing?.id === user.id ? (
              <form onSubmit={saveEdit} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2.5">
                ...
                <input required minLength={3} value={editLogin} onChange={e=>setEditLogin(e.target.value.toLowerCase())} placeholder="Login" className="p-2.5 rounded-xl border border-stone-300 bg-white text-sm" />
              ...
              <div className="flex flex-col md:flex-row md:items-center gap-3 justify-between">
                <div className="min-w-0">
                  ...
                  <div className="text-xs text-stone-500 mt-1">Login: <strong>{user.login}</strong>{user.email ? ` • ${user.email}` : ' • sem e-mail de recuperação'}</div>
