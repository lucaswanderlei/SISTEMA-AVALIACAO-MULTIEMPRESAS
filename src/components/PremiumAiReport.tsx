...
type Report = {
  ...
  period:{days:number;start:string;end:string}; totals:{current:number;previous:number;average:number|null;previousAverage:number|null};
...
const range=(r:Report)=>`${date(r.period.start)} a ${date(new Date(new Date(r.period.end).getTime()-1).toISOString())}`;
...
export function PremiumAiReport({canGenerate}:{canGenerate:boolean}) {
  const [days,setDays]=useState(30),[status,setStatus]=useState<Status|null>(null);
  ...
  const generate=async(refresh=false)=>{
    ...
    try{
      const body=await request('/api/ai/reports',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({days,refresh})});
  ...
  return <section className="space-y-5">
    ...
    {status&&<div className="bg-white border border-stone-200 rounded-2xl p-5 flex flex-wrap items-end gap-3">
      <label className="text-xs font-bold text-stone-600">Período da análise
        <select value={days} disabled={busy} onChange={e=>setDays(Number(e.target.value))} className="block mt-1 rounded-xl border border-stone-300 bg-white text-stone-900 px-3 py-2.5 text-sm">
      ...
      {canGenerate&&<button disabled={busy||!status.configured} onClick={()=>void generate()} className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-rose-600 text-white font-bold text-sm disabled:opacity-50"><Sparkles className="w-4 h-4"/>{busy?'Analisando avaliações...':'Gerar análise'}</button>}
      {canGenerate&&report&&<button disabled={busy||!status.configured} onClick={()=>void generate(true)} className="flex items-center gap-2 px-4 py-2.5 rounded-xl border border-stone-300 text-stone-700 font-bold text-sm disabled:opacity-50"><RefreshCw className="w-4 h-4"/>Gerar novamente</button>}
